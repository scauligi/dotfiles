...
function! s:MakeDictionaryForPhp()
  ...
  "let ddd = system('echo sed -i -e "' . c[1] . ' s#\(.\{'. (c[2] - 1) .'\}\)\(.*\)#\1JUJUMARKER\2#g" ' . fn . ' > /tmp/cmd.cm')
  "let dd3 = system('echo ' . g:pfff_client . ' -vim_auto_complete ' . fn . ' (pipe_to) ' . &dictionary . ' >> /tmp/cmd.cm')
